...
async def insert(event):
    try:
        ...
        await client.send_message(
            SENDER, "<b>Conversation Terminated✔️</b>", parse_mode="html"
        )
        return
...
async def update(event):
    try:
        ...
        sql_command = (
            "UPDATE orders SET id=?, product=?, quantity=?, LAST_EDIT=? WHERE id =?"
        )
        crsr.execute(sql_command, params)  # Execute the query
        conn.commit()  # Commit the changes
...
if __name__ == "__main__":
    try:
        ...
        db_name = "helpers/test-database.db"  # Insert the database name. Database is the folder
